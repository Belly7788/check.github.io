...
// Function to generate 1000 purchase orders
const generatePurchaseOrders = () => {
    ...
    for (let i = 1; i <= 1000; i++) {
        const po = {
            id: i,
            photo: imagePaths[Math.floor(Math.random() * imagePaths.length)],
            code: `PO${String(i).padStart(3, '0')}`, // e.g., PO001, PO002, ..., PO1000
            name: `${itemCategories[Math.floor(Math.random() * itemCategories.length)]} ${i}`,
            date: getRandomDate('2024-01-01', '2025-05-13'),
            amount: (50 + Math.random() * 4950).toFixed(2), // Random between 50.00 and 5000.00
            remark: remarks[Math.floor(Math.random() * remarks.length)],
            rating: Math.floor(Math.random() * 5) + 1, // Random 1 to 5
            orderChecked: Math.random() > 0.5 // Random true/false
        };
...
// Static data for purchase orders
const staticPOs = generatePurchaseOrders();

// Static pagination data
const staticPagination = {
    currentPage: 1,
    perPage: 10,
    total: 1000, // Updated to reflect 1000 entries
};

export default function ListPO({ darkMode }) {
    const { t } = useTranslation();

    // State declarations
    const [purchaseOrders, setPurchaseOrders] = useState(staticPOs);
    const [pagination] = useState(staticPagination);
    const [currentPage, setCurrentPage] = useState(pagination.currentPage);
    const [entriesPerPage, setEntriesPerPage] = useState(pagination.perPage);
    const [searchQuery, setSearchQuery] = useState(''); // For main search
    const [columnSearchQuery, setColumnSearchQuery] = useState(''); // For column search in settings popup
    ...
    const [checkedRatings, setCheckedRatings] = useState([1, 2, 3, 4, 5]);
    ...
    const [checkedOrderStatuses, setCheckedOrderStatuses] = useState([true, false]);
    ...
    const sortData = (field, direction) => {
        const sortedPOs = [...purchaseOrders].sort((a, b) => {
            ...
            if (field === 'amount' || field === 'rating') {
                ...
                valueA = a.orderChecked;
                valueB = b.orderChecked;
            } else {
    ...
    const toggleSettingPopup = (e) => {
        ...
        if (isSettingPopupOpen) {
            ...
            setTimeout(() => {
                ...
                setColumnSearchQuery(''); // Reset column search when closing
            }, 200);
        } else {
    ...
    const toggleOrderCheckbox = (id) => {
        setPurchaseOrders((prev) =>
            prev.map((po) =>
                po.id === id ? { ...po, orderChecked: !po.orderChecked } : po
            )
        );
    };
    ...
    const filteredPurchaseOrders = purchaseOrders.filter((po) => {
        ...
        return (
            checkedRatings.includes(po.rating) &&
            dateMatch &&
            checkedOrderStatuses.includes(po.orderChecked)
        );
    });
                                    ...
                                    >
                                        {visibleColumns.checkbox && (
                                            ...
                                            >
                                                <input
                                                    type="checkbox"
                                                    checked={selectedPOs.length === filteredPurchaseOrders.length}
                                                ...
                                                filteredPurchaseOrders.slice(
                                                    ...
                                                    >
                                                        ...
                                                        {visibleColumns.code && (
                                                            ...
                                                            >
                                                                {po.code ? (
                                                                    <Clipboard darkMode={darkMode} textToCopy={po.code}>
                                                                        <Bellypopover darkMode={darkMode}>
                                                                            <span
                                                                                className={`label-Purple ${darkMode ? 'label-Purple-darkmode' : ''}`}
                                                                                data-belly-caption={po.code}
                                                                            >
                                                                                {po.code &&
                                                                                typeof po.code === "string" &&
                                                                                po.code.length > 15
                                                                                    ? `${po.code.substring(0, 12)}...`
                                                                                    : po.code || ""}
                                                                            </span>
                                                                        </Bellypopover>
                                                                    </Clipboard>
                                                        ...
                                                        {visibleColumns.name && (
                                                            ...
                                                            >
                                                                {po.name ? (
                                                                    <Clipboard darkMode={darkMode} textToCopy={po.name}>
                                                                        <Bellypopover darkMode={darkMode}>
                                                                            <span
                                                                                ...
                                                                                data-belly-caption={po.name}
                                                                            >
                                                                                {po.name &&
                                                                                typeof po.name === "string" &&
                                                                                po.name.length > 15
                                                                                    ? `${po.name.substring(0, 12)}...`
                                                                                    : po.name || ""}
                                                                            </span>
                                                                        </Bellypopover>
                                                                    </Clipboard>
                                                        ...
                                                        {visibleColumns.amount && (
                                                            ...
                                                            >
                                                                {po.amount &&
                                                                typeof po.amount === "string" &&
                                                                po.amount.length > 15
                                                                    ? `${po.amount.substring(0, 12)}...`
                                                                    : po.amount || ""}
                                                            </td>
                                                        )}
                                                        ...
                                                        {visibleColumns.order && (
                                                            ...
                                                            >
                                                                <input
                                                                    type="checkbox"
                                                                    checked={po.orderChecked}
                                                                    onChange={() => toggleOrderCheckbox(po.id)}
                    ...
                    >
                        <div className="p-3">
                            ...
                            <button
                                ...
                                className={`mt-4 w-full text-center px-4 py-2 text-sm font-medium rounded-md transition-colors duration-150 ${
                                    darkMode
                        ...
                        <div className="p-3">
                            ...
                            {[true, false].map((status) => (
                                ...
                                >
                                    <input
                                        type="checkbox"
                                        checked={checkedOrderStatuses.includes(status)}
                                        onChange={() => handleOrderCheckboxChange(status)}
                                        className="rounded h-4 w-4 text-orange-500 focus:ring-orange-400 border-gray-300"
                                    />
                                    <span>{status ? t("ordered") : t("not_ordered")}</span>
                                </label>
                            ))}
                            <button
