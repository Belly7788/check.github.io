...
import { showConfirmAlert } from "../Component/Confirm-Alert/Confirm-Alert"; // Import showConfirmAlert

export function Layout({ children }) {
    const { props } = usePage();
    const { auth } = props; // Access the auth prop
    const user = auth?.user || {}; // Get the user object (username, image)
    ...
    // Get the first letter of the username (or a default if no username)
    const firstLetter = user.username ? user.username.charAt(0).toUpperCase() : "U";

    // Apply dark mode to the document root and save to localStorage
    useEffect(() => {
        if (darkMode) {
    ...
    // Update activeMenu and auto-expand parent menus based on URL
    useEffect(() => {
        setActiveMenu(url);
    ...
    const title = React.Children.map(children, (child) => child.type.title || "Dashboard")[0];
    ...
    return (
        <div className={`flex h-screen ${darkMode ? "dark:text-white" : "text-black"}`}>
            ...
            <div className="flex-1 w-full flex flex-col overflow-hidden">
                                ...
                                {profileOpen && (
                                    ...
                                    >
                                        ...
                                        <button
                                            ...
                                            className={`w-full text-start block px-4 py-2 text-sm ${
                                                darkMode ? "hover:bg-gray-600" : "hover:bg-gray-100"
                                            }`}
                                        >
                                            Sign out
                ...
                {/* Page Content */}
                <main
                    className={`flex-1 min-w-full custom-scrollbar p-2 overflow-y-auto custom-scrollbar ${
